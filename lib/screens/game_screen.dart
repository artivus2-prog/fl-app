...
class _GameScreenState extends State<GameScreen>
  ...
  void _botMove() {
    ...
    if (!skipNormalNextTurn && _gameState.pendingResponsePlayer == null) {
      _gameState.nextTurn();
    } else if (skipNormalNextTurn && _gameState.pendingResponsePlayer == null) {
      _gameState.nextTurn();
    }
    setState(() => _updateTurn());
    _broadcastState();
  ...
  void _onCardTap(UnoCard card) {
    ...
    final isPendingDraw2 = _gameState.pendingResponsePlayer == widget.playerName &&
  ...
  void _confirmMultiPlay() {
    ...
    _executePlayCard(_gameState.currentHand(widget.playerName)
        .where((c) => _selectedCardIds.contains(c.id)).toList());
  }
  ...
  void _drawCard() {
    ...
    if (_gameState.pendingResponsePlayer == widget.playerName) {
      _gameState.playerHands[widget.playerName]!.addAll(_deck.drawMultiple(_gameState.pendingDrawCount ?? 6));
      _gameState.pendingResponsePlayer = null;
    } else {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () { Navigator.pop(ctx); if (widget.isHost) { setState(() { _deck = UnoDeck(seed: Random().nextInt(99999)); _startGameAsHost(); }); } }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF7C4DFF)), child: const Text('Играть снова')),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        if (isPendingResponse) _buildHint('Ответьте своей +2 или возьмите 6 карт!', Colors.orange, Icons.warning),
        if (_noPlayableCards && _isMyTurn) _buildHint('Нет доступных карт — нажмите на колоду', Colors.yellow, Icons.touch_app),
        ...
        _buildPlayerHand(myHand, isPendingResponse),
      ]),
    );
  }

  AppBar _buildAppBar(bool isPendingResponse) {
    return AppBar(
      ...
      actions: [
        ...
        if (isPendingResponse) _buildChip('ОТВЕТЬТЕ!', Colors.orange, false),
        if (_isMyTurn && !isPendingResponse) _buildChip('ВАШ ХОД', const Color(0xFF00E676), false),
      ],
    );
  }
  ...
  Widget _buildPlayerBar() {
    return Container(
      height: 50, margin: const EdgeInsets.symmetric(horizontal: 8),
  ...
  Widget _buildPlayerHand(List<UnoCard> myHand, bool isPendingResponse) {
    if (myHand.isEmpty) {
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Opacity(
                        ...
                        child: Stack(children: [
                          ...
                          if (isPendingDraw2)
                            Positioned(top: -5, right: -5, child: Container(padding: const EdgeInsets.all(4), decoration: const BoxDecoration(color: Colors.green, shape: BoxShape.circle), child: const Icon(Icons.reply, size: 16, color: Colors.white))),
                          if (isSelected)
  ...
  Widget _buildCardWidget(UnoCard card, {bool big = false}) {
    ...
    return Container(
      ...
      child: Stack(children: [
        if (!isWild && !isClear)
          Center(child: Transform.rotate(angle: -0.3, child: Container(width: w * 0.7, height: h * 0.55, decoration: BoxDecoration(color: Colors.white.withOpacity(0.25), borderRadius: BorderRadius.circular(40))))),
        ...
        if (card.type == CardType.number)
          Positioned(top: 6, left: 10, child: Text('${card.number}', style: TextStyle(fontSize: big ? 20 : 14, fontWeight: FontWeight.w900, color: txtColor))),
        if (isWild)
          Positioned(bottom: 0, left: 4, right: 4, child: Container(height: 5, decoration: const BoxDecoration(gradient: LinearGradient(colors: [Colors.red, Colors.yellow, Colors.green, Colors.blue]), borderRadius: BorderRadius.only(bottomLeft: Radius.circular(12), bottomRight: Radius.circular(12))))),
      ]),
    );
  }
